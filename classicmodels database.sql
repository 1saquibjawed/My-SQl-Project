...
and creditlimit between 50000 and 100000  order by creditlimit desc;
...
FROM
    Employees 
LEFT JOIN
    Customer c ON e.employeenumber = c.employeenumber
GROUP BY
...
SELECT
    ...
    SUM(od.quantity_ordered) AS TotalQuantities,
    IFNULL(s.balance_stock, 0) AS TotalQuantitiesInStock,
    IFNULL(s.balance_stock - SUM(od.quantity_ordered), s.balance_stock) AS LeftoverQuantities
FROM
    Customers c
JOIN
...
LEFT JOIN
    Stock  ON p.product_id = s.product_id
GROUP BY
    c.customer_id, p.product_id, s.balance_stock
ORDER BY
...
drop table brand;
insert into brand (laptopName) values('dell'),('hp');
...
insert into colours(colourName) values('white');
insert into colours(colourName) values('silver');
select* from colours; 

select *from brand,colours;
-- ======================================================
...
select p1.fullName as employeeName ,p2.fullName as ManagerName from
project  as p1 left join project as p2
on p1.ManagerID = p2.EmployeeID; 
...
-- alter table  facility add  auto_increment (Facility_ID) WHERE EXTRA;
...
select trim  (1, "       Pune          University     ");
SELECT trim (2, "  Mumbai          University     ");
SELECT trim (3, "     Delhi   University     ");
select  trim (4, "Madras University");
select trim (5, "Nagpur University");
...
BEGIN
    ...
    -- Calculate the years and months difference
...
SELECT
    MIN(count_of_products) AS min_count,
    MAX(count_of_products) AS max_count
FROM
    (
        SELECT
            orderNumber,
...
-- Insert the data into the Emp_BIT table
